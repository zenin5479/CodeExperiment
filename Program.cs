...
namespace CodeExperiment
{
   internal class Program
   {
      ...
      public static void FileWriteAllLines(double[,] arrDou)
      {
         //double[,] arrayDouble =
         //{
         ...
         Console.BackgroundColor = ConsoleColor.DarkBlue;
         StringBuilder stringModified = new StringBuilder();
         string[] arrayString = new string[arrDou.GetLength(0)];
         int row = 0;
         int column = 0;
         while (row < arrDou.GetLength(0))
         {
            while (column < arrDou.GetLength(1))
            {
               if (column != arrDou.GetLength(1) - 1)
               {
                  stringModified.Append(arrDou[row, column] + " ");
                  column++;
               }
               else
               {
